...
import * as peerBanksApi from '@/lib/api/peerBanks'
import type { PeerBank } from '@/types/peerBank'

jest.mock('@/lib/api/peerBanks')
...
beforeEach(() => {
  jest.clearAllMocks()
  jest.mocked(peerBanksApi.getPeerBanks).mockResolvedValue({ peer_banks: mockPeerBanks })
  jest.mocked(peerBanksApi.createPeerBank).mockResolvedValue(mockPeerBanks[0])
  jest.mocked(peerBanksApi.updatePeerBank).mockResolvedValue(mockPeerBanks[0])
  jest.mocked(peerBanksApi.deletePeerBank).mockResolvedValue(undefined)
})

describe('AdminPeerBanksPage', () => {
  it('renders page title and Add button', async () => {
    renderWithProviders(<AdminPeerBanksPage />)
  ...
  it('shows loading spinner while data loads', () => {
    jest.mocked(peerBanksApi.getPeerBanks).mockReturnValue(new Promise(() => {}))
    renderWithProviders(<AdminPeerBanksPage />)
  ...
  it('opens the Add dialog and POSTs a valid payload', async () => {
    ...
    await waitFor(() => {
      expect(peerBanksApi.createPeerBank).toHaveBeenCalledWith({
        bank_code: '444',
        routing_number: 444,
  ...
  it('rejects an invalid base URL in the Add dialog', async () => {
    ...
    expect(peerBanksApi.createPeerBank).not.toHaveBeenCalled()
  })
  ...
  it('toggles a peer bank active flag via the row Disable button', async () => {
    ...
    await waitFor(() => {
      expect(peerBanksApi.updatePeerBank).toHaveBeenCalledWith(1, { active: false })
    })
  })
  ...
  it('removes a peer bank after confirmation', async () => {
    ...
    await waitFor(() => {
      expect(peerBanksApi.deletePeerBank).toHaveBeenCalledWith(1)
    })
  })
  ...
  it('opens the Edit dialog and PUTs base_url + active', async () => {
    ...
    await waitFor(() => {
      expect(peerBanksApi.updatePeerBank).toHaveBeenCalledWith(1, {
        active: true,
